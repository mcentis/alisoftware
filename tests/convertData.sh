...
#usage . ./convertData directory maxProc
...
#the binary files are expected to have the .dat extension
...
do
    ...
    do
	#echo $iBias
	for iRun in $(ls -d $iBias/*.dat)
	do
	    iConf=$(ls -d $iBias/*.cfg)
	    xterm -e "cd $PWD; pwd; ./executables/dataToRoot $iRun $iConf" &
