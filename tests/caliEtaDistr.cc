...
void caliEtaDistr(const char* inFile,double timeCut1 = 0, double timeCut2 = 115,  int polarity = 1, const char* borderStripsFile = "")
{
  ...
  std::cout << "Everything done with the calibrated signal " << std::endl;
  ...
  cooked->SetBranchAddress("caliSignal", signal); // assignment of the calibrated signal
  ...
  cooked->SetBranchStatus("caliSignal", 1);
  ...
  for(long int i = 0; i < nEntries; i++)
      ...
      for(int iCl = 0; iCl < nClust; ++iCl)
	{
	  ...
	  for(unsigned int iSt = 0; iSt < clu.strips.size(); ++iSt)
	    if(fabs(clu.qStrips.at(iSt)) > fabs(seedPH))
	      {
	  	seedPH = clu.qStrips.at(iSt);
	  ...
	  for(unsigned int iSt = 0; iSt < clu.strips.size(); ++iSt) // fill the graphs
		...
		{
		  d1L->Fill(clu.qStrips.at(iSt));
		  leftRatio->Fill(clu.qStrips.at(iSt) / seedPH);
		}

	      if(clu.strips.at(iSt) == (seedNum + 1)) // right first neighbour
		{
		  d1R->Fill(clu.qStrips.at(iSt));
		  rightRatio->Fill(clu.qStrips.at(iSt) / seedPH);
		}

	      clusterShapePH->Fill(clu.strips.at(iSt) - seedNum, clu.qStrips.at(iSt));
	    }
